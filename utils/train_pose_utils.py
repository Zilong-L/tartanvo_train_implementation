...
def calculate_loss( relative_motion, motions_gt,device_id='cuda:0'):
...
def get_loaders(posefile_path, datastr, height, width, batch_size,  flow_only=True, rcr_type="NO_RCR",shuffle=False,rank=None,world_size=None):
    scene_datasets = []
    print(posefile_path)
    ...
    for dataset in scene_datasets:
        dataloaders.append(DataLoader(dataset,sampler=DistributedSampler(dataset=dataset, num_replicas=world_size, rank=rank, shuffle=shuffle),batch_size=batch_size))
    return dataloaders
