...
def calculate_whole_loss(relative_motion, motions_gt,flow_prediction,flow_gt,lambda_flow=0.1,device_id='cuda:0'):
...
def process_whole_sample(ddp_model,sample,lambda_flow,device_id):
    ...
    total_loss,flow_loss,pose_loss,trans_loss,rot_loss = calculate_whole_loss(relative_motion, motions_gt,flow,flow_gt,lambda_flow,device_id)
    return total_loss,flow_loss,pose_loss,trans_loss,rot_loss
...
def process_flowpose_sample(ddp_model,sample,device_id):
    ...
    # forward------------------------------------------------------------------
    flow, relative_motion = ddp_model(flow_input)
